...
function editNode() {
	$.ajax({
		url : "${siteContext}/admin/quartz/start?rid"+ Math.random(), 
		type : "POST",
		dataType : "json",
		async : false,
		cache : false,
	...
	function saveNode() {
		$.ajax({
			url : "${siteContext}/admin/quartz/stop?rid"+ Math.random(), 
			type : "POST",
			dataType : "json",
			async : false,
			cache : false,
				...
				.datagrid(
						{
							...
							onBeforeLoad : function(param) {
								$(this).datagrid('options').url = '${siteContext}/admin/quartz/isRunning?rid'+ Math.random();
							}
						});
	});
</script>
...
<body>
		...
		<div style="margin-bottom: 5px" style="display: none;">
					<a href="javascript:editNode()" class="easyui-linkbutton">运行</a> 
					<a href="javascript:saveNode()" class="easyui-linkbutton">停止</a> 
					<a href="javascript:filterNode()" class="easyui-linkbutton">查询状态</a> 					
