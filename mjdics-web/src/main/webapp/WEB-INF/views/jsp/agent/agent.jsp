...
document.getElementById('popIframe').style.display='none';
	...
	function saveNode() {
		...
		if (node) {
			...
			if(!flag){
				$.messager.alert('提示信息', "数据非法，请按照页面要求修改数据！", 'error');
				return;
			}
	...
	function showDetail(){
		...
		if (node) {
			...
			$("#name").val(node.name);
			$("#agentId").val(node.agentId);
			$("#tel").val(node.tel);
			$("#address").val(node.address);
			$("#paraAgent").val(node.paraAgent);
			$("#money").val(node.money);
			$("#freezeMoney").val(node.freezeMoney);
			$("#bank").val(node.bank);
			$("#bankCard").val(node.bankCard);
		}
	}
		...
		$('#tt').datagrid({
			...
			columns : [ [ {
				...
				formatter:function(value,row,index){
					return '<a href="${siteContext}/admin/agent/selectChildAgent?agentId='+value+'">'+value+'</a>';
				}
			},{
				field : 'name',
				title : '经销商姓名',
				...
				formatter : function(value) {
					<c:forEach var="agentTyep" items="${agentTyepEnum}" varStatus="i"> 
						if ('${agentTyep}' == value)
							return '${agentTyep.name}'; 
					</c:forEach>
				}
			},
			...
			onBeforeLoad:function(param){
				...
				var agentId = '${agentId}';
				if(agentId){
					$(this).datagrid('options').url = '${siteContext}/admin/agent/selectAgent?paraAgent='+agentId;
				}else{
					$(this).datagrid('options').url = '${siteContext}/admin/agent/selectAgent?type='+type;
				}
			}
		});
	});
	...
	function addAgent(){
		$.ajax({
			url : "${siteContext}/admin/agent/insertAgent?type=${type}",
			type : "POST",
...
<body>
		...
		<div style="margin-bottom: 5px">
				<c:if test="${(sessionScope.agent.type=='ADMIN'&&type=='FIRST_BUSS')||sessionScope.agent.type!='ADMIN'}">
					<a  href="javascript:newNode('${sessionScope.agent.type}')" class="easyui-linkbutton">新增</a> 
		...
		<tr>
			<td>
				...
				<div id="detail" class="easyui-dialog"
					...
					closed="true">
					<form id="fmEdit" class="registerform"
						...
						<table width="100%" style="table-layout: fixed;">
							<tbody>
								<tr>
									...
									<td width="80px" align="right">代理商ID：</td>
									<td width="270px"><input id="agentId" type="text" name="agentId" value=""
										class="inputxt" datatype="s4-10" nullmsg="请输代理商Id" onchange="isExistAgent(this);"
										errormsg="格式为4-10位字符" /></td>
									<td><div class="Validform_checktip"></div></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td align="right">电话：</td>
									<td><input id="tel" type="text" name="tel" value=""
										class="inputxt" datatype="s11-11" nullmsg="请输入联系电话"
										errormsg="长度为11位" /></td>
									<td><div class="Validform_checktip"></div></td>
								</tr>
								<tr>
								...
								<tr>
									<td colspan="4" style="vertical-align:middle; text-align:center;"><input name="Submit"
										type="button" onclick="addAgent();" value="添加" /></td>
								</tr>
							</tbody>
						</table>
					</form>
