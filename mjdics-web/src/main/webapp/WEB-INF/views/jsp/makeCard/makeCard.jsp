...
document.getElementById('popIframe').style.display='none';
	...
	function exportExcelByMakeId() {
			var node = $('#tt').datagrid('getSelected');
			var str="${siteContext}/admin/makeCard/exportExcelByMakeId?id="+node.id+"&t="+ Math.random();
	...
	function makeSuccess() {
		var node = $('#tt').datagrid('getSelected');
		$.ajax({
			url : "${siteContext}/admin/makeCard/updateMakeCard?id="+node.id,
			type : "POST",
		...
		$('#tt').datagrid({
			...
			columns : [ [ {
				...
				formatter:function(value,row,index){
					return '<a href="${siteContext}/admin/card/selectCard?makeId='+row.id+'">'+value+'</a>';
				}
				...
				formatter : function(value) {
					<c:forEach var="makeCard" items="${makeCardStatusEnum}" varStatus="i"> 
						if ('${makeCard}' == value)
				...
				formatter:function(value,row,index){
					if(row.status=='MAKE')
						return '<a href="javascript:makeSuccess()">制卡完成</a> | <a href="javascript:exportExcelByMakeId()">导出制作卡数据</a> ';
			...
			onBeforeLoad:function(param){
				//type = '${type}';
				$(this).datagrid('options').url = '${siteContext}/admin/makeCard/selectMakeCard';
			}
		});
	});
	function initMakeCard(){
		$('#makeCard').dialog('open').dialog('setTitle', '申请申请制卡');
	}
	function makeCard(){
		$.ajax({
			url : "${siteContext}/admin/makeCard/insertMakeCard?",
			type : "POST",
			data : $('#applyForm').serialize() ,
			success : function(text) {
				$('#makeCard').dialog('close');
				$.messager.alert('提示信息', text, 'info');
				$('#tt').datagrid("reload");
			}
		});
	...
	function exportExcel(){
		var str="${siteContext}/admin/makeCard/insertMakeCard?t="+ Math.random();
		window.location.href=encodeURI(str);
	}
</script>
</head>
...
<!-- 					<a  href="javascript:initMakeCard()" class="easyui-linkbutton">制作卡</a> 
			...
			<a href="javascript:exportExcel()" class="easyui-linkbutton" iconCls="icon-search" style="padding:2px, 10px">导出制作卡</a>
				</div>
</div>
	...
	<table height=100% width="100%" cellSpacing=0 cellPadding=0 id=table0>
		<tr>
			<td>
				...
				<div id="makeCard" class="easyui-dialog"
					...
					<form id="applyForm" class="registerform"
						...
						<table width="100%" style="table-layout: fixed;">
							<tbody>
								<tr>
									<td class="need">*</td>
									<td align="right">卡片数量：</td>
									<td>
									  <input id="amount" type="text" name="amount" value="" class="inputxt" nullmsg="请输入申请的卡片数量" />
									</td>
									...
									<td>
										...
										errormsg="请选择互卡类型" style="height: 26px;">
											<c:forEach items="${cardTypeList}" var="cardType">
													<option value="${cardType.name}#${cardType.cardTypeId}">${cardType.name}</option>
											</c:forEach>
										</select>									
									</td>
									<td><div class="Validform_checktip"></div></td>
								...
								<tr>
									<td colspan="4" style="vertical-align:middle; text-align:center;"><input name="Submit"
										type="button" onclick="makeCard();" value="确定" /></td>
								</tr>
							</tbody>
						</table>
					</form>
