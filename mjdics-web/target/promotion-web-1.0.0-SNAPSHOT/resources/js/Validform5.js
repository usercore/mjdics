...
(function($,win,undef){
	...
	var Validform=function(forms,settings,inited){
		...
		forms.each(function(n){
			...
			//表单元素日期比较时的信息提示增强;
			Validform.util.pastCheckEnhance.call($this);
	...
	Validform.util={
		...
		pastCheckEnhance:function(){
			var curform=this;
			curform.find("input[pastcheck]").each(function(){
				var _this=$(this);
				var pastcheckinput=curform.find("input[name='"+$(this).attr("pastcheck")+"']");
				_this.bind("blur",function(){
					if(_this.val()!="" && pastcheckinput.val()!=""){
						var d1 = new Date(_this.val().replace(/\-/g, "\/"));
						var d2 = new Date(pastcheckinput.val().replace(/\-/g, "\/"));
						if(d2 > d1){
							if(_this.attr("tip")){
								if(_this.attr("tip") == _this.val()){return false;}	
							}
			...
			*/
			...
			if(Validform.util.toString.call(Validform.util.dataType[datatype])=="[object Function]"){
				...
				if(passed === true){
					...
					if(obj.attr("pastcheck")){
						var theother=curform.find("input[name='"+obj.attr("pastcheck")+"']:first");

						var d1 = new Date(obj.val().replace(/\-/g, "\/"));
						var d2 = new Date(theother.val().replace(/\-/g, "\/"));
						if(d2 > d1){
							passed=false;
							type=3;
							info=obj.attr("errormsg")  || curform.data("tipmsg").w || tipmsg.w;
